function [M,bMap]= binaryMap6DOFR3SL1(H,xmax,xmin,ymax,ymin,zmax,zmin,deltaX,deltaY,deltaZ,angleX,angleY,angleZ)
    
    %Randomized DH standard parameter for manipulator
    D=[0 0 rand()/2 rand()/2 0 0];
    A=[0 rand()/2 rand()/2 0 0 0];
    ...
    for k=1:sizeZ
       for j=1:sizeY
          for i=1:sizeX  
             try
                if all(not(isnan(m.ikine6s(  trotx(angleX) * troty(angleY) * trotz(angleZ)* transl(H{i,j,k})  ))))
