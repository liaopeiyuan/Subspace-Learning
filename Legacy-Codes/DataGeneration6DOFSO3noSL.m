...
%clear all; 
function []=DataGeneration6DOFSO3noSL(n,entry,point)
...
point            = [ -1+2*rand() -1+2*rand() -1+2*rand()];
...
%Initializing
[ M,                        bMap ]  = binaryMap6DOFSO3noSL(H,xmax,xmin,ymax,ymin,zmax,zmin,deltaX,deltaY,deltaZ,dispX,dispY,dispZ);
...
filename = horzcat('6DOFSO3noSL.','n',num2str(n),'.','L',num2str(l),'.',datestr(datetime("now"),'mm.dd.yy.HH.MM'),'.mat');
